...
function addNewCase($premierValue, $qmonitor, $isNew){
	$product=productValue($_POST["displayStatus"]);
	/*if ($_SESSION['DBtoUse']== 'TICO_DB_FT'){
		//if ($qmonitor==$_POST["engineerlst"]){
			if($_POST["ascallback"]=='QM'){
				$product=0.33;}
			}*/
		//}
	//Mar-8-2013 --Added by Marcos Ramírez
        if ($isNew===0){$product=0;}//Product value, it must be 0 when the case is new and assigned as callback or elevation
	$conn = connectToDB();//Connection stablished
		if(!$conn )
			die( print_r( sqlsrv_errors(), true));
               
	$query = "exec uspAddCase ".$_POST["severitylst"].",".$premierValue.",".$_POST["displayStatus"].",".$_POST["casetxt"].",".$qmonitor.",".$_POST["engineerlst"].",1,".$isNew.", ".$product ;
...
function updateCase($caseValue, $qmonitor, $engineer, $tipo){
	...
	$conn = connectToDB();//Connection stablished
		if(!$conn )
			die( print_r( sqlsrv_errors(), true));
	if($tipo=="Callback"){
		$query = "exec uspAsCallback ".$caseValue.",".$qmonitor.",".$engineer.", 1, ".$_POST["displayStatus"].", ".$product;
	}
	if($tipo=="Elevation"){
		$query = "exec uspAsElevation ".$caseValue.",".$qmonitor.",".$engineer.", 1, ".$_POST["displayStatus"].", ".$product;
	}
