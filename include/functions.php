...
//	echo implode(",", $getAllProductOnMyProfile);
	...
	while($row = sqlsrv_fetch_array($getAllProductOnMyProfile) ){
		...
		$productsValue[$x] = "<input name=\"Prodcuts\" title=\"".$row["productDesc"]."\" type=\"checkbox\" value=\"".$row["productId"]."\" Checked> ".$row["productDesc"];	
		}
		else{
			
			$productsValue[$x] = "<input name=\"Prodcuts\" title=\"".$row["productDesc"]."\" type=\"checkbox\" value=\"".$row["productId"]."\"> ".$row["productDesc"];	
			}
...
function getAllProductsSupportedOnMyProfile($usrId){
	...
	while($row = sqlsrv_fetch_array($getAllSupportedProductOnMyProfile) ){
		$productsSupported[$x] = "<input name=\"Prodcuts\" title=\"".$row["productDesc"]."\" type=\"checkbox\" value=\"".$row["productId"]."\"> ".$row["productDesc"];	
//		<input name="" title="$row["productDesc"]" type="checkbox" value="$row["productDesc"]">
		$x++;	
		}
		closeDBConnetion();
...
function compareArrayProducts($userId){
	...
	//	}	
	///$results = array_merge($supported, $difference);
	return $ListOfSupportedProducts;
}
...
function getNextSchedExcep($ursId){
	...
	if ( $getNextSchedExcep === false){ 
		...
		while($row = sqlsrv_fetch_array($getNextSchedExcep)){
			...
			<input name=\"X\" type=\"submit\" value=\"X\"><INPUT size=\"1\" TYPE=\"text\" name=\"excId\" value=\"".$row["schedExecId"]."\" Style=\"visibility:hidden\"  > ". $row["Reason"]. " From: " . getHoursFromDate($row["timeFrom"], u) . " To " . getHoursFromDate($row["timeTo"], u) . "</form> <br/>";	
			$x++;
		}
	}
	return $result;
