...
if ($row = sqlsrv_fetch_array($getCase)){
	...
	if ($row = sqlsrv_fetch_array($getUsers)){
		...
		if($_POST["engineerlst"] != $row['userId']){ //If the assignee has changed
			$queryNew = "exec uspModifyCaseNewEng ". $_POST["casetxt"].",".$QMDetails[0].",1,".$_POST["engineerlst"].",".$_POST["severitylst"].",".$topremier.", ". $_POST["displayStatus"];
			...
			else{echo "The case " .$_POST["casetxt"]. " has been updated succesfully";}
		}//if assignee changed
		else{
			$queryNew = "exec uspModifyCaseSameEng ". $_POST["casetxt"].",".$QMDetails[0].",".$_POST["engineerlst"].",".$_POST["severitylst"].",".$topremier.", 1, ". $_POST["displayStatus"];
	...
	else{echo "The case is not assigned";}
