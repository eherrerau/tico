...
if ($row = sqlsrv_fetch_array($result)){ //For each Engineer
	...
	if ($rowCases = sqlsrv_fetch_array($getCases)){
		...
		echo "<div id=row><div id=\"engineer\"> Total Cases last 60 days: </div><div id=\"graphline\" style=\"width:". $tamano."px; \"></div><div id=\"tamano\"  style=\"position:relative; \" >". $rowCases['totalcases']."</div></div>";
		$tamano2= $rowCases['average']*5;
		echo "<div id=row><div id=\"engineer\"> W/days last 60 days: </div><div id=\"tamano\"  style=\"position:relative; \" >". round($rowCases['totalWorkingDays'],2)."</div></div>";
	...
	echo "<div id=row><div id=\"engineer\"> My average Last 60 days: </div><div id=\"graphline\" style=\"width:". $tamano2."px; \"></div><div id=\"tamano\"  style=\"position:relative; \" >". round($rowCases['average'],2)."</div></div>";
	}
	$x = $x+1;
	$serie ="Serie". $x;
}//if
...
if ($totalUsers>0){
	...
	echo "<div id=row><div id=\"engineer\">Team Avg last 60 days</div><div id=\"graphline\" style=\"width:".round($totalAvg*5,2)."px; \"></div><div id=\"tamano\"  style=\"position:relative; \" >".round($totalAvg,2)."</div></div>";
